...
int main()
{
    ...
    alloc.size = 128*1024;
    alloc.physical = 0;
    int ret = ioctl(fd, JZ_VPU_IOCTL_ALLOC, &alloc);
    if(ret < 0)
    {
    ...
    printf("Loading code...\n");
    int cfd = open("test3_p1.bin", O_RDONLY);
    if(cfd < 0)
    {
    ...
    int ok = 1;
    for(int x=0; x<alloc.size/4; ++x)
    {
        if(data[x] != (x*x ^ 0xaaaa5555))
        {
            printf("Mismatch at offset %i\n", x);
            ok = 0;
        }
    }
    if(ok)
